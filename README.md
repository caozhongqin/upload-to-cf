...
5. 点 **添加键值对**，写入你的密码：
   - 键名：`sk-你自己设的复杂密码`（请用强密码，不要用示例值）
   - 值：`active`
6. 点 **保存**

> ✅ 现在你的记事本里应该有 3 个 ID：**D1 数据库 ID** 和 **KV 命名空间 ID**（R2 只需要名字 `upload-files`，不需要 ID）
...
去 KV 命名空间里删掉旧的键值对，添加一个新的就行。

### 怎么添加多个密码？

在 KV 里添加多个键值对，键名是密码，值都是 `active`。每个密码都能用。
